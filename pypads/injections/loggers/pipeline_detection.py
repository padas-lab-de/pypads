...
class PipelineTracker(InjectionLogger):
    ...
    def __pre__(self, ctx, *args, _logger_call: LoggerCall, _pypads_pipeline_type="normal", _pypads_pipeline_args=False,
                **kwargs):
        ...
        node_id = _to_node_id(_logger_call.call.call_id.wrappee, ctx)
        label = _to_node_label(_logger_call.call.call_id.wrappee, ctx)
        if not network.has_node(node_id):
            network.add_node(node_id, label=label)

        label = _to_edge_label(_logger_call.call.call_id.wrappee, _pypads_pipeline_args, args, kwargs)
    ...
    def __post__(self, ctx, *args, _pypads_pipeline_args=False, _logger_call: LoggerCall, _pypads_pre_return, **kwargs):
        from pypads.app.pypads import get_current_pads
        pads = get_current_pads()
        ...
        label = "return " + _to_edge_label(_logger_call.call.call_id.wrappee, _pypads_pipeline_args, args, kwargs)
        if pads.call_tracker.call_depth() == 1:
